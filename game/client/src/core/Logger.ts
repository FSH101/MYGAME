...
export class Logger {
  ...
  private emit(level: LogLevel, message: string, details?: unknown): void {
    ...
    switch (level) {
      case "info":
        console.log(`[ИНФО] ${message}`, details ?? "");
        break;
      case "warn":
        console.warn(`[ПРЕДУПРЕЖДЕНИЕ] ${message}`, details ?? "");
        break;
      case "error":
...
export function setupGlobalErrorLogging(): void {
  window.addEventListener(
    "error",
    (event) => {
      logger.error(`Неперехваченное исключение: ${event.message}`, event.error ?? event.message);
    },
    { passive: true },
  );
